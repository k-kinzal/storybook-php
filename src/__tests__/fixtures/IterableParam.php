...
/**
 * Demonstrates iterable type hint in parameters.
 * The `iterable` type accepts both arrays and Traversable objects.
 */
class IterableParam {
    ...
    public function render(iterable $items, string $style = 'list'): string {
        ...
        if (empty($collected)) {
            return "<div class=\"iterable-empty\" style=\"padding: 16px; color: #9ca3af; font-family: system-ui;\">{$this->emptyMessage}</div>";
        }
        ...
        if ($style === 'grid') {
            ...
            return "{$header}<div class=\"iterable-grid\" style=\"display: flex; flex-wrap: wrap; gap: 8px; font-family: system-ui;\">{$cards}</div>";
        }

        $lis = implode('', array_map(fn($item) => "<li style=\"padding: 4px 0;\">{$item}</li>", $collected));
        return "{$header}<ul class=\"iterable-list\" style=\"margin: 0; padding-left: 20px; font-family: system-ui;\">{$lis}</ul>";
    }
}
