...
/**
 * Showcases a variety of default value types in a single constructor:
 ...
 * - enum case
 * - new expression
 * Tests that the parser correctly extracts all default types and the
 * runner correctly instantiates with each.
 */
enum Theme: string {
    case Light = 'light';
    case Dark = 'dark';
...
class MixedDefaultsShowcase {
    ...
    public function render(): string {
        ...
        return <<<HTML
        <div class="showcase" style="padding: 16px; background: {$s['bg']}; color: {$s['text']}; border: 1px solid {$s['border']}; border-radius: 12px; font-family: system-ui; opacity: {$vis};">
