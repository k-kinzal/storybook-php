...
enum CssColor: string implements Renderable {
    ...
    public function render(): string {
        ...
        return <<<HTML
        <div class="css-color" style="display: inline-flex; align-items: center; gap: 12px; padding: 12px 20px; border-radius: 8px; background: {$this->value}; color: {$textColor}; font-family: monospace;">
    ...
    public function swatch(int $size = 48): string {
        ...
        <div class="swatch" style="display: inline-flex; flex-direction: column; align-items: center; gap: 4px;">
            ...
            <span style="font-size: 11px; color: #6b7280; font-family: monospace;">{$this->value}</span>
        </div>
        HTML;
    }
}
